...
class TestEventDetection(unittest.TestCase):
    """Sanity tests for the postprocessing.event_detection submodule of the resurfemg library."""
    ...
    width = fs // 2
    prominence = 0.5 * (
        np.nanpercentile(breathing_signal - baseline, 75)
        + np.nanpercentile(breathing_signal - baseline, 50)
    )

    peak_idxs, _ = scipy.signal.find_peaks(
        breathing_signal, height=treshold, prominence=prominence, width=width
    )

    def test_baseline_crossing_starts(self):
        """Test that the onoffpeak_baseline_crossing function returns the same number of start indices as peaks."""
        peak_start_idxs, _, _, _, _ = evt.onoffpeak_baseline_crossing(
            self.breathing_signal, self.baseline, self.peak_idxs
        )

        assert len(self.peak_idxs) == len(peak_start_idxs)

    def test_baseline_crossing_ends(self):
        """Test that the onoffpeak_baseline_crossing function returns the same number of end indices as peaks."""
        _, peak_end_idxs, _, _, _ = evt.onoffpeak_baseline_crossing(
            self.breathing_signal, self.baseline, self.peak_idxs
        )

        assert len(self.peak_idxs) == len(peak_end_idxs)

    def test_slope_extrapolate_starts(self):
        """Test that the onoffpeak_slope_extrapolation function returns the same number of start indices as peaks."""
    ...
    def test_slope_extrapolate_ends(self):
        """Test that the onoffpeak_slope_extrapolation function returns the same number of end indices as peaks."""
...
class TestPoccDetection(unittest.TestCase):
    ...
    def test_baseline_crossing_starts(self):
        """Test that the find_occluded_breaths function returns the same number of peak indices as valid peaks."""
...
class TestFindLinkedPeaks(unittest.TestCase):
    ...
    def test_find_linked_peaks(self):
        """Test the find_linked_peaks function."""
        t_1 = [10.0, 15.0, 20.0]
        t_2 = [x + 0.2 for x in range(30)]
        linked_peaks = evt.find_linked_peaks(t_1, t_2)
        np.testing.assert_array_equal(linked_peaks, np.array([10, 15, 20]))
...
class TestDetectEmgBreaths(unittest.TestCase):
    ...
    def test_detect_emg_breaths(self):
        """Test the detect_emg_breaths function."""
        detected_peaks = evt.detect_emg_breaths(y_env_emg, y_emg_baseline)
        np.testing.assert_array_equal(detected_peaks, peaks_env)
