...
class TestZeroCrossingFlow(unittest.TestCase):
    def test_zero_crossing_flow(self):
        ...
        zc_expected = np.arange(99, 400, 100)
        np.testing.assert_array_equal(zc, zc_expected)
        zc_candidate_expected = np.arange(49, 500, 50)
...
class TestVolumeComputation(unittest.TestCase):
    def setUp(self):
        ...
        self.zc, self.zc_candidate = pneu.zero_cros_flow(
    ...
    def test_volume_computation_last_point(self):
        volume, _, volume_raw, ee_idxs = pneu.volume_computation(
            ...
            zc=self.zc,
            method="Last point")
        np.testing.assert_array_equal(ee_idxs, self.zc)
        # End-expiratory is determined by flow zero crossings
        # Volume should be zero at the zero crossings next sample due to
        # integration
        v_ee = volume_raw[ee_idxs+1]
    ...
    def test_volume_computation_last_points(self):
        ...
        for _zc in self.zc:
            zc_begin = max(0, _zc - int(0.1 * self.fs))
            zc_array = np.arange(zc_begin, _zc + 1)
            ee_idxs_pred = np.concatenate((ee_idxs_pred, zc_array))
            vol_pred = np.arange(11.0, 0.0, -1.0) / 100.0  # 0.11 to 0.01 L
        ...
        # 0.11 to 0.01 L in the 0.1 s before the zero crossing.
