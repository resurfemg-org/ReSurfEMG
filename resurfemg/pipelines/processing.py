...
def quick_look(
    ...
    fs_emg: int,
    plot_raw: bool = False,
    plot_clean: bool = True,
    plot_env: bool = True,
    plot_power_spectrum: bool = True,
    ...
    if plot_power_spectrum:
        ...
        vis.show_power_spectrum(emg_raw, fs_emg, int(t_emg[-1]), signal_unit="uV")
        axis_f.grid(True)
...
def ecg_removal_gating(
    emg_raw: np.ndarray,
    ecg_peaks_idxs: np.ndarray,
    gate_width_samples: int,
    method: int = 3,
    ecg_shift: int | None = None,
) -> np.ndarray:
