...
class TimeSeries:
    ...
    def __getitem__(self, key: str):
        ...
        :returns y_data: data of the selected signal type. May be a
            numpy array or a tuple of two numpy arrays for confidence
            interval data (lower, upper).
        :rtype y_data: ~numpy.ndarray | tuple
        """
    ...
    def __setitem__(self, key: str, value: np.ndarray | tuple[np.ndarray, np.ndarray]):
        ...
        or a custom signal type
        :type key: str
        :param value: data of the selected signal type. Can be a numpy
            array or a tuple of two numpy arrays (lower_ci, upper_ci).
        :type value: ~numpy.ndarray | tuple
    ...
    def signal_type_data(self, signal_type: str | None = None) -> np.ndarray:
        ...
        y_data = np.zeros(self._y_data["raw"].shape)
    ...
    def envelope(
        ...
        **kwargs,
        ...
        See preprocessing.envelope submodule. The envelope is by default stored in
        ...
        if env_window is None:
            if "fs" in self.param:
                ...
                msg = "Evelope window and sampling rate are not defined."
                raise ValueError(msg)
    ...
    def baseline(
        ...
        **kwargs,
        ...
        See postprocessing.baseline submodule. The baseline is stored in the
    ...
    def detect_emg_breaths(
        ...
        See postprocessing.event_detection submodule. The peaks are stored in the
        ...
        if (
            end_idx is not None and end_idx > len(self[signal_io[0][0]])
        ...
        self.set_peaks(
            peak_idxs=peak_idxs,
            signal=self[signal_io[0][0]],
            peak_set_name=peak_set_name,
            overwrite=overwrite,
        )

    def link_peak_set(
        self, peak_set_name, t_reference_peaks, linked_peak_set_name=None
    ):
    ...
    def plot_full(
        ...
        **kwargs,
        ...
        :param plot_ci: plot the confidence interval of the envelope
        :type plot_ci: bool
    ...
    def plot_markers(
        self, peak_set_name, axes, valid_only=False, colors=None, markers=None
    ):
        ...
        def get_color_marker(values, default):
            ...
            if isinstance(values, list):
                return (
                    values[:3] if len(values) > 2 else [values[0], values[1], values[1]]
                )
            msg = f"Invalid {default}"
            raise ValueError(msg)
    ...
    def plot_curve_fits(self, peak_set_name, axes, valid_only=False, colors=None):
...
class TimeSeriesGroup:
    ...
    def __init__(self, y_raw, t_data=None, fs=None, labels=None, units=None):
        ...
        if t_data is None and fs is None:
            ...
            t_data = np.array([x_i / fs for x_i in range(self.param["n_samp"])])

        if labels is None:
            self.labels = self.param["n_channel"] * [None]
        else:
            if len(labels) != self.param["n_channel"]:
                msg_0 = "Number of labels does not match the number of data channels."
                raise ValueError(msg_0)
            self.labels = labels
        ...
        for method_name in cls._available_methods:
            if method_name not in cls.__dict__:  # don"t overwrite explicit definitions
                ...
                def make_wrapper(name):
                    def wrapper(self, channel_idxs=None, **kw):
                        return self.run_wrapper(name, channel_idxs=channel_idxs, **kw)
    ...
    def to_numpy(self, channel_idxs=None, signal_io=(None,)):
        ...
        return np.array(
            [self.channels[idx].signal_type_data(signal_io[0]) for idx in channel_idxs]
        )

    def _check_plot_kwargs(self, method, channel_idxs, kwargs):
    ...
    def run_wrapper(self, method, channel_idxs=None, **kwargs):
        if channel_idxs is None:
            channel_idxs = np.arange(self.param["n_channel"])
        elif isinstance(channel_idxs, int):
            channel_idxs = np.array([channel_idxs])
    ...
    if TYPE_CHECKING:
        ...
        def envelope(self, channel_idxs=None, **kwargs): ...
        def baseline(self, channel_idxs=None, **kwargs): ...
        def plot_full(self, channel_idxs=None, **kwargs): ...
        def plot_peaks(self, channel_idxs=None, **kwargs): ...
        def plot_markers(self, channel_idxs=None, **kwargs): ...
        def set_peaks(self, channel_idxs=None, **kwargs): ...
...
class EmgDataGroup(TimeSeriesGroup):
    ...
    _available_methods = TimeSeriesGroup._available_methods + [
        "filter_emg",
        "get_ecg_peaks",
        "gating",
        "wavelet_denoising",
    ...
    def _check_ecg_future_warning(self, channel_idxs, kwargs):
        """Handle FutureWarning and auto-ECG-peak resolution for gating/
        wavelet_denoising.
        """
        if all(
            ...
            if "ecg_peak_idxs" in kwargs:
                ...
                self.run_wrapper("set_peaks", **ecg_kwargs)
            else:
                ecg_kwargs = {
                    "ecg_raw": kwargs.pop("ecg_raw", None),
                    "bp_filter": kwargs.pop("bp_filter", True),
                }
                self.run_wrapper("get_ecg_peaks", **ecg_kwargs)

    def _resolve_ecg_source(self, kwargs):
        """Handle ECG source resolution logic, extracted from run()."""
        if "ecg_raw" in kwargs and kwargs["ecg_raw"] is not None:
            ...
            print("Channel raw signals used for ECG peak detection.")

    # override auto-generated wrappers for methods needing special handling
    def get_ecg_peaks(self, channel_idxs=None, **kwargs):
        self._resolve_ecg_source(kwargs)
        return self.run_wrapper("get_ecg_peaks", channel_idxs=channel_idxs, **kwargs)

    def gating(self, channel_idxs=None, **kwargs):
        self._check_ecg_future_warning(channel_idxs, kwargs)
        return self.run_wrapper("gating", channel_idxs=channel_idxs, **kwargs)

    def wavelet_denoising(self, channel_idxs=None, **kwargs):
        self._check_ecg_future_warning(channel_idxs, kwargs)
        return self.run_wrapper(
            "wavelet_denoising", channel_idxs=channel_idxs, **kwargs
        )

    if TYPE_CHECKING:

        def filter_emg(self, channel_idxs=None, **kwargs): ...
        def get_ecg_peaks(self, channel_idxs=None, **kwargs): ...
        def gating(self, channel_idxs=None, **kwargs): ...
        def wavelet_denoising(self, channel_idxs=None, **kwargs): ...
